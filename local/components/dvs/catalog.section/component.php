...
if (isset($_REQUEST[$arParams["ACTION_VARIABLE"]]) && isset($_REQUEST[$arParams["PRODUCT_ID_VARIABLE"]])) {
    ...
    if (($action == "ADD2BASKET" || $action == "BUY" || $action == "SUBSCRIBE_PRODUCT") && $productID > 0) {
        if (Loader::includeModule("sale") && Loader::includeModule("catalog")) {
            ...
            if ($addByAjax) {
                if ($successfulAdd) {
                    $addResult = ['STATUS' => 'OK', 'MESSAGE' => GetMessage('CATALOG_SUCCESSFUL_ADD_TO_BASKET')];
                } else {
                    $addResult = ['STATUS' => 'ERROR', 'MESSAGE' => $strError];
                }
 ...
 * Work with cache
 *************************************************************************/
if ($this->startResultCache(false, [$arrFilter, ($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()), $arNavigation, $pagerParameters])) {
    if (!Loader::includeModule("iblock")) {
    ...
    $existIblock = Iblock\IblockSiteTable::getList([
        'select' => ['IBLOCK_ID'],
        'filter' => ['IBLOCK_ID' => $arParams['IBLOCK_ID'], '=SITE_ID' => SITE_ID, '=IBLOCK.ACTIVE' => 'Y']
    ])->fetch();
    ...
    if ($bCatalog) {
        ...
        $arResultModules['currency'] = true;
        if (is_array($arParams["IBLOCK_ID"]) && !empty($arParams['IBLOCK_ID'])) {
    ...
    //EXECUTE
	if(CSite::InDir("/brands/"))
	{
		$arSort = array(
			"CATALOG_AVAILABLE" => "desc,nulls",
			"SORT" => "asc",
			"PROPERTY_HIT" => "desc",
			"PROPERTY_NEW" => "desc",
			//"PROPERTY_SORT_ALL" => "asc",
		);
	}
    ...
    if (!empty($arResult["ELEMENTS"]) && ($bGetProperties || ($bCatalog && $boolNeedCatalogCache))) {
        ...
        CIBlockElement::GetPropertyValuesArray($arElementLink, 12, $arPropFilter);
    ...
    if ($bCatalog && $boolNeedCatalogCache && !empty($arResult["ELEMENTS"])) {
        ...
        CCatalogDiscount::SetDiscountProductCache($arResult["ELEMENTS"], ['IBLOCK_ID' => $arParams["IBLOCK_ID"], 'GET_BY_ID' => 'Y']);
    }
    ...
    elseif (!is_array($arParams["OFFERS_FIELD_CODE"]))
        $arParams["OFFERS_FIELD_CODE"] = [$arParams["OFFERS_FIELD_CODE"]];
    ...
    elseif (!is_array($arParams["OFFERS_PROPERTY_CODE"]))
        $arParams["OFFERS_PROPERTY_CODE"] = [$arParams["OFFERS_PROPERTY_CODE"]];
