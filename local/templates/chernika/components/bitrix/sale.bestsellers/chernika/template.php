...
<?foreach($arResult["ITEMS"] as $key => $arItem):
    ...
    <a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="product-link" rel="nofollow">
        ...
        <span class="product-price js-prod_price">
        	<?//$arItem['OFFERS'][0]['MIN_PRICE']['VALUE']?>
            <?if ( $arItem['OFFERS'][0]['MIN_PRICE']['VALUE'] > 0 ) {
                if(SITE_ID!='m1') {
                    if(SITE_ID!='m2') {
                        ?>
                        <span class="product-price_old"><?=number_format($arItem['OFFERS'][0]['MIN_PRICE']['VALUE'],0,'',' ')?> ₽</span>
                        <?
                    }
                }
                $withoutLenzes = $arItem['PROPERTIES']['DISCONT_WITHOUT_LENZES']['VALUE'];
                if ($withoutLenzes =='Y') {
                    $fullPrice = number_format($arItem['OFFERS'][0]['MIN_PRICE']['VALUE'], 0, '', '');
                    $priceBest =  ceil($fullPrice/2);?>
                    <span class="product-price_current"><?=number_format($priceBest,0,'',' ')?> ₽</span><?
                } else {?>
                    <span class="product-price_current"><?=number_format($arItem['OFFERS'][0]['MIN_PRICE']['DISCOUNT_VALUE'],0,'',' ')?> ₽</span>
                    <?}
                    ?>

                <? } elseif ( $arItem['PROPERTIES']['PRISE']['VALUE'] > 0) { ?>
                    <span><b><?=number_format($arItem['PROPERTIES']['PRISE']['VALUE'],0,'',' ')?> ₽</b></span>
                <? } else { ?>
