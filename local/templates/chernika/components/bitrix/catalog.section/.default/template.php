...
<?if (!empty($arResult['ITEMS']) ):?>
    ...
    <div id="js-ax-ajax-pagination-content-container" class="row">
        ...
        <?if ( $arItem['PROPERTIES']['LIMITED_QUANTITY']['VALUE'] == "Y" ) {?>
            <div class="sale mobile-none"><p>Ограниченное предложение</p></div>
        <? } ?>

        <?
        if ( $_SESSION['GEO_IP']['NAME'] == 'Санкт-Петербург' ) {
            $perc = '50';
        } else {
            $perc = '30';
        }
        ?>                           
            <?if ($arItem['PROPERTIES']['BRAND']['VALUE'] == '5927' && $arItem['IBLOCK_ID'] == '12'
            || $arItem['PROPERTIES']['BRAND']['VALUE'] == '15697' && $arItem['IBLOCK_ID'] == '12'
            || $arItem['PROPERTIES']['BRAND']['VALUE'] == '248335' && $arItem['IBLOCK_ID'] == '12'
            ) {?>
            ...
            <? } ?>
                            ...
                            <?if ( !empty($arItem['OFFERS'][0]['ITEM_PRICES'][0]['PERCENT']) ) {
                                $offerPercent = "";
                                foreach($arItem['OFFERS'] as $arItemOffer)
                                {
                                    if($arItemOffer["CAN_BUY"])
                                    {
										$offerPercent = $arItemOffer["ITEM_PRICES"][0]["PERCENT"];
                                        break;
                                    }
                                }
                            ?>
                            <div class="sale"><p> 
                                <?=($arItem['OFFERS'][0]['ITEM_PRICES'][0]['PERCENT'] <= 50) ? '- '.$offerPercent : 'SALE '.$offerPercent;?>
                            %</p></div>
                            <?}?>
...
?> 
                        ...
                        <a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="product-link">
                            <div class="product-img-wrapper">
                                <img class="product-img js-prod_img lazyload"
                                     ...
                                     data-src="<?= !empty($arItem['PREVIEW_PICTURE']['SRC']) ? $resize_image['src'] : $arItem['DETAIL_PICTURE']['SRC'] ?>"
                                     width="<?= $arItem['PREVIEW_PICTURE']['WIDTH'] ?>"
                                     height="<?= $arItem['PREVIEW_PICTURE']['HEIGHT'] ?>"
                                     alt="<?= $arItem['NAME'] ?>">
                                <noscript>
                                    <img class="product-img js-prod_img" src="<?= $resize_image['src'] ?>" width="<?= $arItem['PREVIEW_PICTURE']['WIDTH'] ?>" height="<?= $arItem['PREVIEW_PICTURE']['HEIGHT'] ?>" alt="<?= $arItem['NAME'] ?>">
                                </noscript>
                            </div>
                            ...
                            <span class="product-price js-prod_price">
                                <? switch ($arItem['PRODUCT_PRICE_TYPE']) {
                                    ...
                                    case 'OLD_PRICE':
                                        if(SITE_ID!='m1') {?>
										<?if($arItem['PRODUCT_AVAILABLE'] == 'PRE_ORDER'):?>
                                            <span class="product-price_current" style="color:#9e9e9e!important;"><?=number_format($arItem['ITEM_PRICES'][0]['PRICE'],0,'',' ')?>&nbsp;₽</span>
                                            <?break;?>
										<?endif;?>
                                        <?
                                        ...
                                        $discont30 = $arItem['PROPERTIES']['DISCONT_WITHOUT_LENZES']['VALUE'];
                                        $fullPrice = number_format($arItem['PROPERTIES']['OLD_PRICE']['VALUE'],0,'','');
                                        if($discont30 == 'Y'){?>
                                            <span class="product-price_current"><?=number_format(ceil($fullPrice/2),0,'',' ')?>&nbsp;₽</span>
                                        <?} else {?>
                                    ...
                                    case 'PRICE':
                                        ?>
                                        <span class="product-price_single"><?=number_format($arItem['ITEM_PRICES'][0]['PRICE'],0,'',' ')?>&nbsp;₽</span>
                                        <?
                                        break;
                                ...
                                <?} elseif ( $arItem['PROPERTIES']['OLD_PRICE']['VALUE'] > 0 && $arItem['CATALOG_QUANTITY'] > 0 ) {?>
                                    <span class="product-price_old"><?=number_format($arItem['PROPERTIES']['OLD_PRICE']['VALUE'],0,'',' ')?>&nbsp;₽</span>
                                    <span class="product-price_current"><?=number_format($arItem['ITEM_PRICES'][0]['PRICE'],0,'',' ')?>&nbsp;₽</span>

                                <? //} elseif ( $arItem['ITEM_PRICES'][0]['PRICE'] > 0 && $arItem['CATALOG_QUANTITY'] > 0 ) { ?>
        ...
        <?
            ...
            if ($arItem['OFFERS'][0]['ITEM_PRICES'][0]['PERCENT'] > 25) {?>
<div class="col-6 col-sm-6 col-md-4 col-lg-3<?if($arParams['LINE_ELEMENT_COUNT']==3) echo ' col-xl-4';?>">
                <div class="catalog-item ">
                    <div class="product js-prod_card">
                        <div class="voblers">
        ...
        ?>                           
            ...
            <? } ?>
                            ...
                            <?if ( !empty($arItem['OFFERS'][0]['ITEM_PRICES'][0]['PERCENT']) ) {?>
                            <div class="sale"><p> 
                                <?=($arItem['OFFERS'][0]['ITEM_PRICES'][0]['PERCENT'] <= 50) ? '- '.$arItem['OFFERS'][0]['ITEM_PRICES'][0]['PERCENT'] : 'SALE '.$arItem['OFFERS'][0]['ITEM_PRICES'][0]['PERCENT'];?>
                            %</p></div>
                            <?}?>
