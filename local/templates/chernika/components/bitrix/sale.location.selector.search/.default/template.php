...
<?else:?>
    ...
    <?CJSCore::Init();?>
    <?$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/js/sale/core_ui_widget.min.js')?>
    <?$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/js/sale/core_ui_etc.min.js')?>
    <?$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/js/sale/core_ui_autocomplete.min.js');?>
    ...
    <div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <?if(strlen($arResult['MODE_CLASSES'])):?> <?=$arResult['MODE_CLASSES']?><?endif?>">
        ...
        <div class="<?=$dropDownBlock?> bx-ui-sls-input-block">
            ...
            <input type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" value="<?=$arResult['VALUE']?>" class="dropdown-field c-input" placeholder="<?=Loc::getMessage('SALE_SLS_INPUT_SOME')?> ..." />
