    ...
    <div class="container">
        ...
        <? } ?>
                        ...
                        <?if ( !empty($arResult['OFFERS'][0]['ITEM_PRICES'][0]['PERCENT']) ) {?>
                            ...
                            <?} else {?>
                                <?if(intval($arResult["ITEM_PRICES"][0]["PERCENT"]) > 0):?>
                                    <div class="sale">
                                        <p><noindex>
                                        <?=$arResult["ITEM_PRICES"][0]["PERCENT"]?>
                                        % <?=\COption::GetOptionString( "askaron.settings", "UF_LABEL_AFTER_PERCENT");?>
                                        </noindex></p>
                                    </div>
                                <?endif;?>
							<? } ?>
                                ...
                                <?if ($arResult['PRODUCT_AVAILABLE'] != 'PRE_ORDER') {?>
                                    <?if(SITE_ID!='m1') {?>
                                        <div class="price">
                                            ...
                                            ?>
                                            <p><span class="price-title qwe"><?=$priceTitle?></span><span class="price-before"><?= number_format($arResult['PROPERTIES']['OLD_PRICE']['VALUE'], 0, '', ' ') ?> <span>₽</span></span><?if($arResult['IBLOCK_ID']==42){?> за одну линзу<?}?></p>
                                        </div>
                                    <?}?>
                                <?}?>
                            <?break;
...
<?switch ($arResult['PRODUCT_PRICE_TYPE']) {
    ...
    <?$phoneNumber = \PDV\Tools::getDataByCity($arParams['CITY_PARAMS']['ID'])['PHONE_HEADER'][0];?>
    <p style="margin-top: 0;">Цена действительна по промокоду: <strong>ЗИМА</strong> и при предварительной записи по телефону: <nobr><a href="tel:<?=\PDV\Tools::clearPhone($phoneNumber)?>"><?=$phoneNumber?></a></nobr></p>
    <?}
break;
}?>
<?*/?>
