...
<div class="login-form-socials-block">
    <?
    if($arParams["~FOR_SPLIT"] == 'Y'):?>
        <?foreach($arAuthServices as $service):
            ...
            if ( $service['ICON'] == 'facebook' ) {
                $cl = ' class="fb"';
                $text = '<i class="fab fa-facebook-f"></i>';
            }
            elseif ( $service['ICON'] == 'vkontakte' ) {
                $cl = ' class="vk"';
                $text = '<i class="fab fa-vk"></i>';
            }
            elseif ( $service['ICON'] == 'google' ) {
                $cl = ' class="google"';
                $text = '<i class="fab fa-google-plus-g"></i>';
            }
            else
                $text = '<i class="bx-ss-icon '.htmlspecialcharsbx($service["ICON"]).'"></i>';

        if(($arParams["~FOR_SPLIT"] == 'Y') && (is_array($service["FORM_HTML"])))
            $onClickEvent = $service["FORM_HTML"]["ON_CLICK"];
        elseif(($arParams["~FOR_SPLIT"] == 'Y') && !empty($service["ONCLICK"]))
            $onClickEvent = 'onclick="'.$service["ONCLICK"].'"';
        else
            $onClickEvent = "onclick=\"BxShowAuthService('".$service['ID']."', '".$arParams['SUFFIX']."')\"";
        ?>
            <a title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?> id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"<?=$cl?>><?=$text?></a>
        <?endforeach?>
    <?endif;?>

	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>" style="display: none;">
