...
<div id="<?echo $CONTAINER_ID?>" class="bx-searchtitle <?=$arResult["VISUAL_PARAMS"]["THEME_CLASS"]?>">
	<form action="<?echo $arResult["FORM_ACTION"]?>">
			...
			<input id="<?echo $INPUT_ID?>" placeholder="<?=$arParams["INPUT_PLACEHOLDER"]?>" type="text" name="q" value="<?=htmlspecialcharsbx($_REQUEST["q"])?>" autocomplete="off" >
				...
				<span class="bx-searchtitle-preloader <?if($arParams["SHOW_LOADING_ANIMATE"] == 'Y') echo 'view';?>" id="<?echo $PRELOADER_ID?>"></span><button id="<?echo $INPUT_ID?>_reset" type="reset" style="display:none;"><svg class="modal-close-icon"><use xlink:href="#close"></use></svg></button>
				<button class="" type="submit" name="s"><span></span><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="18" height="18" viewbox="0 0 18 18">
                                    <defs>
...
<?if($arResult["VISUAL_PARAMS"]["THEME_COLOR"]):?>
	<style>.bx_smart_searche .bx_item_block.all_result {display:none!important;}
...
//	BX.ready(function(){
    $(document).ready(function() {
		...
		$("#<?echo $INPUT_ID?>").on("click change keyup input paste", function()
		{
			$("#<?echo $INPUT_ID?>_reset").hide();
			if($("#<?echo $INPUT_ID?>").val() != "") $("#<?echo $INPUT_ID?>_reset").show();
		});
		$("#<?echo $INPUT_ID?>_reset").on("click", function()
		{
			$("#<?echo $INPUT_ID?>_reset").hide();
		});
	});
